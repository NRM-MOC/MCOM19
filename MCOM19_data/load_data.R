...
bio_data <- read_excel("data/esbase/marina18fisk.xlsx",
  ...
  mutate(ACCNR = trans_accnr(ACCNR_ESBASE),
         ...
         YWD = str_sub(ACCNR, 4, 5), # Extracts year from accession number (MoCiS does not need week/day)
         ...
         Season = ifelse(ACCNR %in% paste0("C2018/0", 7173:7255), "H", Season), 
         YWD = ifelse(ACCNR %in% paste0("C2018/0", 4029:4148), "18", YWD), 
...
#
full_data <- bind_rows(historical_hav, historical_uria, new_data) %>% 
  mutate('FPRC*' = ifelse(is.na(.[['FPRC*']]), FPRC, .[['FPRC*']]),
         LTPRC = ifelse(is.na(LTPRC), .[['LTPRC*']], .[['LTPRC*']]))
write_csv(full_data, "clean_data/full_data.csv")
