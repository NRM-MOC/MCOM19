...
load("../MCOM19_MoCiS/mocis_all.Rdata")
...
basin_fits <- basin_data %>% 
  ...
  mutate(lm_fit = map(data, ~lm(log(value) ~ LOC + YEAR, data = .x)),
         ...
         lm_predict = pmap(list(lm_intercept, lm_slope, lm_slope_lower, lm_slope_upper), 
                           ~tibble(YEAR = seq(2009, 2018, by = 0.5), CONC = exp(..1 + ..2 * YEAR),
...
basin_met <- basin_plot(basin_plot_data, grp = "Metals", unit = bquote("Concentration,"~mu*g/g))
...
iso_data <- read_csv("../MCOM19_data/clean_data/full_data.csv", guess_max = 100000) %>% 
