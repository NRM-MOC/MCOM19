...
save_tables <- function(mocis_object, report_year = 2018){
...
plot_mean_maps <- function(mocis_object, report_year = 2018){
...
plot_rate_maps <- function(mocis_object, report_year = 2018){
  data <- mocis_tibble(mocis_object) %>% filter(gen == "CLUP", loc != "ORFJ")
  if (nrow(data) == 0) return()
  world_map <- ne_countries(scale = "medium", returnclass = "sf") %>% 
    st_crop(c(xmin=8, xmax=25, ymin=54, ymax=70))
